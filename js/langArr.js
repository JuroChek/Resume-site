const langArr = {
    "title": {
        "de": "Lebenslauf",
        "en": "Resume",
        "ru": "Резюме",
    },
    "head-text": {
        "de": "Lebenslauf",
        "en": "Resume",
        "ru": "Резюме",
    },

    "fname": {
        "de": "<b>Farbitnyi Yurii</b>",
        "en": "<b>Farbitnyi Yurii</b>",
        "ru": "<b>Фарбитный Юрий</b>",

    },

    "name": {
        "de": "<b>Yurii</b>",
        "en": "<b>Yurii</b>",
        "ru": "<b>Юрий</b>",
    },

    "kontakts-heater": {
        "de": "<b>Kontaktdaten</b>",
        "en": "<b>Contacts</b>",
        "ru": "<b>Контакты</b>",
    },

    "phone": {
        "de": "Telefon",
        "en": "Phone",
        "ru": "Телефон",
    },

    "adress": {
        "de": "Adresse",
        "en": "Address",
        "ru": "Адрес",
    },

    "job-h": {
        "de": "<b>Über mich</b>",
        "en": "<b>About me</b>",
        "ru": "<b>О себе</b>",
    },

    "job-1": {
        "de": "Ich suche einen Job als Frontend- oder Backend- <b>JavaScript Entwickler</b>. Dieses Projekt ist meine <b>Präsentationsarbeit</b>. Ich werde dieses Projekt ständig weiterentwickeln und verbessern. Nach Abschluss des JavaScript-Kurses werde ich HTML und CSS lernen. Erfahrung mit der Arbeit in einem Team. Ich bin zielstrebig, nicht konfliktscheu und habe Erfahrung als Führungskraft.",
        "en": "I'm looking for a Frontend or Backend <b>JavaScript developer</b> job. This project is my <b>presentation work</b>. I will continue to develop and improve this project as my experience improves. Upon completion of the JavaScript course, I will move on to a detailed study of HTML and CSS. Experience of working in a team. Goal-oriented, non-conflictive, has experience as a manager.",
        "ru": "Я ищу работу Frontend или Backend <b>JavaScript developer.</b> Данный проект является моей <b>презентационной работой</b>. Данный проект я планирую постоянно развивать и совершествовать по мере улучшения моих знаний. По завершении курса JavaScript перейду к детальному изучению HTML и CSS. Умею работать в коллективе. Целеустремленный, не конфликтный, есть опыт руководителя.",
    },
    "job-h1": {
        "de": "<b>Für den Arbeitgeber</b>",
        "en": `<b>For the employer</b>`,
        "ru": `<b>К работодателю</b>`,
    },
    "job-2": {
        "de": "Ich würde mich gerne an der von Ihnen gestellten Testaufgabe versuchen. Bitte kontaktieren Sie mich per E-Mail.",
        "en": "I would be happy to try my hand at the test assignment you have provided. Please contact me by e-mail.",
        "ru": "Я буду рад попробовать свои силы в выполнении тестового задания. Пожалуйста, свяжитесь со мной по e-mail.",
    },

    "studium-heater": {
        "de": "Ausbildung",
        "en": "Education",
        "ru": "Образование",
    },

    "language": {
        "de": "Sprachen",
        "en": "Languages",
        "ru": "Языки",
    },

    "studium1": {
        "de": "College Bauwesen und Architektur / Nationale Universität für Bauwesen und Architektur Charkiw",
        "en": "College based on Kharkiv State University of Construction and Architecture",
        "ru": "Лицей на базе ХГТУСА",
    },

    "studium2": {
        "de": "Nationale Universität für Bauwesen und Architektur Charkiw",
        "en": "Kharkiv National University of Construction and Architecture",
        "ru": "Харьковский государственный национальный университет строительсва и архитектуры",
    },
    "studium21": {
        "de": "Abschluss, Diplom",
        "en": "Diploma",
        "ru": "Диплом специалиста",
    },

    "studium3": {
        "de": "Deutschkurs VHS",
        "en": "German course VHS",
        "ru": "Курс немецкого языка на VHS",
    },

    "studium5": {
        "de": "Russisch, Ukrainisch, Englisch (A1), Deutsch (A2)",
        "en": "Russian, Ukrainian, English(A1), German(A2)",
        "ru": "Русский, Украинский, Английский(A1), Немецкий(A2)",
    },
    
    "expiriens": {
        "de": "Berufserfahrung",
        "en": "Job experience",
        "ru": "Опыт работы",
    },
   
    "job1": {
        "de": "Servicemetall. Herstellung von Metallkonstruktionen für Aufzüge",
        "en": "Servicemetall. The company producing metal structures for elevators",
        "ru": "Сервисметал. Компании по производству запастных частей для Лифтов",
    },

    "job2": {
        "de": "als Abteilungsleiter.",
        "en": "as department head.",
        "ru": "Должность начальник отдела.",
    },

    "job3": {
        "de": "Aufgaben:",
        "en": "Tasks:",
        "ru": "Задания:",
    },

    "job4": {
        "de": "- Unternehmensleitung;",
        "en": "- company management;",
        "ru": "- управление отделом;",
    },
    "job5": {
        "de": "- Arbeit mit Finanzen;",
        "en": "- work with finances;",
        "ru": "- работа с финансами;",
    },
    "job6": {
        "de": "- Außendienst",
        "en": "- External sales",
        "ru": "- внешние продажи;",
    },
    "job7": {
        "de": "- Site leiter;",
        "en": "- Site manager;",
        "ru": "- Модератор сайта;",
    },
    "job8": {
        "de": "- Arbeit mit verschiedenen  Metallbearbeitungswerkzeugen;",
        "en": "- work with sophisticated metalworking equipment;",
        "ru": "- работа с различным металлообрабатывающим оборудованием;",
    },
    "job9": {
        "de": "- Abkanten, Hacken, Schneiden, Stanzen, Bearbeiten;",
        "en": "- folding, chopping, cutting, punching, machining;",
        "ru": "- гибка, рубка, резка, пробивка, мехобработка;",
    },
    "job10": {
        "de": "- Pulverbeschichten;",
        "en": "- powder coating;",
        "ru": "- системы порошковой окраски изделий;",
    },
    "job11": {
        "de": "- Anordnung der Prozessroute;",
        "en": "- arrangement of the process route;",
        "ru": "- организация технологического маршрута изготовления изделий",
    },
    "job12": {
        "de": "- Entwicklung der Designdokumentation;",
        "en": "- Development of design documentation;",
        "ru": "- Разработка проектно-конструкторской документации;",
    },
    "job13": {
        "de": "Ingenieurbüro TEP-Sojus.",
        "en": "Engineering company TEP-Soyuz.",
        "ru": "Инжиниринговая фирма ТЭП-Союз.",
    },

    "job14": {
        "de": "als Ingenieur",
        "en": "engineer",
        "ru": "Должность инженер 2 категории",
    },
    "job15": {
        "de": "Aufgaben:",
        "en": "Tasks:",
        "ru": "Задания:",
    },
    "job16": {
        "de": "- Entwicklung der Designdokumentation;",
        "en": "- Development of design documentation;",
        "ru": "- Разработка проектно-конструкторской документации;",
    },
    "job17": {
        "de": "- Arbeit mit Computernetzwerken",
        "en": "- Work with computer networks",
        "ru": "- работа с компьютерными сетями;",
    },
    "job18": {
        "de": "- Arbeiten mit Computer-Aided-Design-Systemen",
        "en": "- Working with computer-aided design systems",
        "ru": "- работа с системами автоматизированного проектирования;",
    },
    "job19": {
        "de": "- Geschäftskorrespondenz;",
        "en": "- business correspondence;",
        "ru": "- деловая переписка;",
    },
    "job20": {
        "de": "- Dokumentenfluss;",
        "en": "- document flow;",
        "ru": "- документооборот;",
    },

    "job21": {
        "de": "ISP PSN",
        "en": "ISP PSN",
        "ru": "Итернет-провайдер PSN",
    },
    "job22": {
        "de": "Monteur",
        "en": "Fitter",
        "ru": "Техник-монтажник",
    },
    "job23": {
        "de": "Aufgaben:",
        "en": "Tasks:",
        "ru": "Задания:",
    },
    "job24": {
        "de": "- Verlegung von Internetnetzwerken auf Basis von Twisted Pair;",
        "en": "- laying Internet networks based on twisted pair;",
        "ru": "- прокладка интернет сетей на базе витой пары;",
    },
    "job25": {
        "de": "- Einrichten von Routern;",
        "en": "- setting up routers;",
        "ru": "- настройка роутеров;",
    },
    "job26": {
        "de": "- Konfigurieren von Betriebssystemen für den Netzwerkbetrieb;",
        "en": "- Configuring operating systems for network working;",
        "ru": "- Натройка операционных систем для рабты в сети;",
    },
    "expo": {
        "de": "Erfahrung",
        "en": "Experience",
        "ru": "Навыки",
    },
    "expo-heat": {
        "de": "In der IT",
        "en": "IT Experience",
        "ru": "Опыт в IT",
    },
    "expo1": {
        "de": "Gute Kenntnisse im Bereich IT:",
        "en": "Good knowledge of IT:",
        "ru": "Имею знания в IT:",
    },
    "expo2": {
        "de": "- Frontend-Entwickler, JavaScript, HTML, CSS;",
        "en": "- Frontend developer, JavaScript, HTML, CSS;",
        "ru": "- Frontend developer, JavaScript, HTML, CSS;",
    },
    "expo3": {
        "de": "- Aufbau von Computernetzwerken",
        "en": "- building computer networks.",
        "ru": "- построение компьютерных сетей;",
    },
    "expo4": {
        "de": "- Erfahrung in der Arbeit mit Servern und Domänen;",
        "en": "- experience work with servers, Domains;",
        "ru": "- опыт работы с серверами, доменами;",
    },
    "expo5": {
        "de": "- CMS-spezifische Systeme",
        "en": "- CMS specific systems",
        "ru": "- cистемы управления сайтом CMS;",
    },
    "expo6": {
        "de": "- SEO für Suchmaschinen wie Google.",
        "en": "- SEO for search engines like Google.",
        "ru": "- SEO для поисковой системы Google;",
    },

    "expo7": {
        "de": "- Computersysteme Windows, Linux Ubuntu.",
        "en": "- Computer systems Windows, Linux Ubuntu.",
        "ru": "- Компьютерные системы Windows, Linux Ubuntu.",
    },
    "expo8": {
        "de": "Erfahrung in",
        "en": "Experience in",
        "ru": "Опыт в",
    },
    "expo9": {
        "de": "Metall- verarbeitung",
        "en": "Steel processing",
        "ru": "Метало- обработка",
    },
    "expo10": {
        "de": "Kenntnisse der Metallverarbeitung Technologien. Metallverarbeitungstechnik: Biegen, Schneiden, Zerspanen, Schweißen, etc. Arbeiten mit rostfreiem und normalem Stahl sowie mit Nichteisenmetallen. Nasslackierung und Pulverbeschichtung.",
        "en": "Knowledge of steel processing technologies. Metal processing technologies: bending, cutting, machining, welding, etc. Work with stainless and ordinary steels, as well as non-ferrous metals. wet painting and powder painting.",
        "ru": "<b>Знания в металообработке.</b> Технологии обработки металлов: гибка, резка, мехобработка, сварка и т.д.. Работа с нержавеющими и обычными сталями, а также цветными металлами. Мокрая покраска и порошковая покраска.",
    },
    "expo11": {
        "de": "- Metallverarbeitungstechnik (Biegen, Schneiden, Bearbeiten, Schweißen usw.) ",
        "en": "- steel processing technology (bending, cutting, machining, welding, etc.)",
        "ru": "- технология обработки металлов (гибка, резка, мехобработка, сварка и т.д.);",
    },
    "expo12": {
        "de": "-Arbeiten mit rostfreien und gewöhnlichen Stählen sowie mit Nichteisenmetallen",
        "en": "-Working with stainless and common steels as well as non-ferrous metals",
        "ru": "- работа с нержавеющими и обычными сталями, а также цветными металлами;",
    },
    "expo13": {
        "de": "-Nasslackierung, Pulverlackierung.",
        "en": "-Wet painting, powder painting.",
        "ru": "- мокрая покраска и порошковая покраска;",
    },
    "hobby-heater": {
        "de": "Hobbys",
        "en": "Hobby",
        "ru": "Хобби",
    },
    "hobby-heater1": {
        "de": "Hobbys",
        "en": "Hobby",
        "ru": "Хобби",
    },
    "hobby1": {
        "de": "Bücher lesen, wissenschaftliche und historische Artikel",
        "en": "Reading books, scientific and historical articles",
        "ru": "Чтение книг, научных и исторических статей",
    },
    "hobby2": {
        "de": "Ich fotografiere gern",
        "en": "I like to take pictures",
        "ru": "Люблю фотографировать.",
    },
    "hobby3": {
        "de": "Ich fotografiere, Architektur, Natur und Objecte.",
        "en": "I take pictures of architecture, nature, objects.",
        "ru": "Фотографирую архитектуру, природу, предметы.",
    },
    "hobby4": {
        "de": "Elektronik, Computer, Audiogeräte, Netzwerke",
        "en": "Electronics, computers, audio equipment, Networks",
        "ru": "Увлекаюсь электроникой, компьютерами, аудиотехникой.",
    }, 
    "hobby5": {
        "de": "Autodiagnose und -reparatur.",
        "en": "Car diagnostics and repair.",
        "ru": "Диагностика и ремонт автомобилей.",
    },
    "hobby6": {
        "de": "Restaurierung",
        "en": "Restoration",
        "ru": "Реставрация",
    },
    "info-footer": {
        "de": "Alle Informationen auf dieser Website dienen dazu, jegliche Entwicklung zu verhindern",
        "en": "All information on this site belongs to its developer",
        "ru": "Вся информация на этом сайте пренадлежит ее разработчику",
    },
    "hardware-footer": {
        "de": "Für die Entwicklung der Website wurde ein HP-Laptop mit Intel-Prozessor verwendet.",
        "en": "An HP laptop with an Intel processor was used to develop the site.",
        "ru": "Для разрабтки сайта использовался ноутбук фирмы HP на базе процессора intel.",
    },

    "software-footer": {
        "de": "Für die Entwicklung dieser Website wurden das Betriebssystem Linux Ubuntu 20.04.6 LTS und die kostenlose Software Visual Studio Code (VSC) verwendet.",
        "en": "The Linux Ubuntu 20.04.6 LTS operating system and free Visual Studio Code (VSC) software were used to develop this site.",
        "ru": "Для разрабтки этого сайта была использована операционная система Linux Ubuntu 20.04.6 LTS и бесплатное ПО Visual Studio Code (VSC).",
    },
   

   


};